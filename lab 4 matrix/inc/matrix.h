...
public:
    matrix() = default; //base constr

    matrix(const matrix &current) {  //copy constr
        for (int i = 0; i < N; i++){
            for (int j = 0; j < M; j++){
    ...
    matrix& operator= (const matrix<type, N, M>& current){ //constr prisvaiv
        if (this != &current){
            for (int i = 0; i < N; i++){
                for (int j = 0; j < M; j++){
                    m_matrix[i][j] = current.m_matrix[i][j];
    ...
    type determinant(){
        ...
        if (N == M && M == 2){
            ...
            return static_cast<type>(m_matrix[0][0] * m_matrix[1][1] * m_matrix[2][2] +\
                m_matrix[1][0] * m_matrix[0][2] * m_matrix[2][1] + m_matrix[0][1] * m_matrix[2][0] * m_matrix[1][2]\
                - m_matrix[0][2] * m_matrix[1][1] * m_matrix[2][0] - m_matrix[0][0] * m_matrix[1][2] * m_matrix[2][1]\
                - m_matrix[2][2] * m_matrix[0][1] * m_matrix[1][0]);
            //triangle method
        }
        else if (N == M && M == 1){
