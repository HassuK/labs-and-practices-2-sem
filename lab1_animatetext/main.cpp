...
void textOutput(const animatedText& obj){
    sf::RenderWindow window(sf::VideoMode(800, 600), "animated_text");
    sf::Text SFMLText;
    sf::Font mainFont;
    if (mainFont.loadFromFile("..\\..\\..\\ext\\font\\HARLOWSI.ttf")){
        SFMLText.setFont(mainFont);
        SFMLText.setCharacterSize(40);
        SFMLText.setFillColor(sf::Color::Black);
        SFMLText.setStyle(sf::Text::Regular);
        SFMLText.setPosition(300,300);
    }
    else{
...
int main() {
    ...
    animatedText test(20, "It's Working");
    textOutput(test);
    return 0;
}
